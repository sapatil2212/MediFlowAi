...
export async function reconcileSubscriptionFromCashfree(
  ...
  const paymentMethod =
    cfSub?.authorization_details?.payment_method &&
    typeof cfSub.authorization_details.payment_method === "object"
      ? Object.keys(cfSub.authorization_details.payment_method)[0]
      : cfSub?.authorization_details?.payment_group ||
        cfSub?.authorization_details?.payment_method ||
        null;
  ...
  if (nowActive) {
    ...
    const base =
      nextSchedule && !Number.isNaN(nextSchedule.getTime())
    ...
    if (activated) {
      ...
      await notify(local, {
        ...
        details: [
          ...
          {
            label: "Next renewal",
            value: nextSchedule ? nextSchedule.toLocaleDateString("en-IN") : "—",
          },
        ],
      });
    }
...
export async function recordSubscriptionCharge(
  ...
  await execute(
    ...
    [
      generateId(),
      ref,
      local.cfSubscriptionId,
      local.userId,
      local.tenantId,
      charge.cfPaymentId || null,
      charge.amount,
      local.currency,
      charge.status,
  ...
  if (charge.status === "SUCCESS") {
    const from =
      local.currentPeriodEnd && new Date(local.currentPeriodEnd) > new Date()
 ...
 */
export async function syncSubscriptionPaymentsFromCashfree(
  subscriptionRef: string,
): Promise<number> {
  ...
  for (const p of payments) {
    const cfPaymentId = p?.cf_payment_id
      ? String(p.cf_payment_id)
      : p?.payment_id
        ? String(p.payment_id)
        : null;
    ...
    const status =
      rawStatus === "SUCCESS"
        ? "SUCCESS"
        : rawStatus === "FAILED" || rawStatus === "USER_DROPPED"
          ? "FAILED"
          : "PENDING";
    ...
    const paymentMethod =
      (auth.payment_method && typeof auth.payment_method === "object"
        ? Object.keys(auth.payment_method)[0]
        : null) ||
      auth.payment_group ||
      null;
    ...
    const paidAt = p?.payment_initiated_date
      ? toMysqlDateTime(new Date(p.payment_initiated_date))
      : null;
    const scheduledAt = p?.payment_schedule_date
      ? toMysqlDateTime(new Date(p.payment_schedule_date))
      : null;
...
export async function handleCashfreeWebhookRequest(request: Request): Promise<Response> {
  ...
  const subscriptionRef =
    subDetails?.subscription_id || data?.subscription_id || payDetails?.subscription_id || null;
  const cfSubscriptionId =
    subDetails?.cf_subscription_id ||
    data?.cf_subscription_id ||
    payDetails?.cf_subscription_id ||
    null;
  ...
  const eventKey =
    [
      eventType,
      cfSubscriptionId || subscriptionRef || "",
      cfPaymentId || "",
      payload?.event_time || "",
    ]
      .filter(Boolean)
    ...
    if (subRef) {
      ...
      if (payDetails) {
        const rawStatus = String(
          payDetails?.payment_status || payDetails?.status || "",
        ).toUpperCase();
        ...
        if (status !== "PENDING" && paymentType !== "AUTH") {
          await recordSubscriptionCharge(subRef, {
            ...
            failureReason:
              payDetails?.failure_details?.failure_reason || payDetails?.payment_message || null,
            scheduledAt: payDetails?.payment_schedule_date
              ? new Date(payDetails.payment_schedule_date)
              : null,
            paidAt: payDetails?.payment_completion_time
              ? new Date(payDetails.payment_completion_time)
              : null,
          });
        }
      }
