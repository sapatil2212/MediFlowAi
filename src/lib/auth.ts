 ...
 * "Sync with Cashfree" action to pull the real, current status + amount +
 * payment mode + failure reason for orders that were never finalized locally
 * (e.g. the webhook is not configured or the user abandoned the return trip).
 *
 * Returns the reconciled ledger status ("SUCCESS" | "FAILED" | "CANCELLED" |
 * "PENDING") or null if the order could not be reconciled. Never throws.
 */
export async function reconcileOrderPaymentHistory(orderId: string): Promise<string | null> {
  ...
  try {
    ...
    // Resolve tenant/user context from the order id (order_renew_<tenantId>_<ts>)
    // falling back to the customer id embedded in the Cashfree order.
    ...
    // Determine the ledger status. Prefer the granular payment-attempt status
    // when present, otherwise fall back to the coarser order-level status.
