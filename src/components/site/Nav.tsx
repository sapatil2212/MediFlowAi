...
export function Nav() {
  ...
  return (
    <div
      className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "px-4 pt-3 pb-1" : ""}`}
    >
      <nav
      ...
      >
        <div
          className={`mx-auto flex items-center justify-between px-6 transition-all duration-300 ${scrolled ? "h-16 max-w-full" : "h-20 max-w-7xl"}`}
        >
          {/* Left side: Logo */}
          ...
          <div className="hidden md:flex flex-initial items-center justify-center gap-1">
            {links.map((l) => (
              <Link
                ...
                inactiveProps={{
                  className: "text-zinc-600 hover:text-zinc-900 hover:bg-zinc-950/[0.03]",
                }}
        ...
        {open && (
          <div
            className={`border-t border-zinc-950/5 bg-white px-6 py-4 md:hidden ${scrolled ? "rounded-b-2xl" : ""}`}
          >
